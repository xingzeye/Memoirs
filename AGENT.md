...
- 侧栏和账号菜单提供备份入口；备份只下载当前账号未进入回收站的回忆、Markdown、JSON 和可读取的原始媒体 ZIP。导出使用临时文件响应以降低大备份内存压力，媒体文件在 ZIP 中原样存储以减少云端 CPU 压缩耗时；Markdown 中图片可内嵌预览，视频保留为链接；缺失原文件的媒体会被跳过并写入 `manifest.json` 的跳过清单。导入只接受本应用导出的 ZIP，会创建当前账号的新回忆和媒体，不覆盖现有内容。
