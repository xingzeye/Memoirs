...
- 2026-09-21：OpenAI Sites 版本从静态入口升级为 Worker 后端，`.openai/hosting.json` 声明 D1/R2 逻辑绑定，`worker/index.js` 提供兼容 React 前端的页面和 JSON API；当前覆盖回忆文本 CRUD、回收站、相册读取、基础媒体上传和 Django 版备份 ZIP 导入，手机扫码上传和 Django Admin 尚未迁移。
...
- 2026-05-10：备份导入前端在提交中会阻止重复提交，成功后清空旧错误提示，避免同时显示失败和成功消息。
