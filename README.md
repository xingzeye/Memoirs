...
- [普通用户使用手册](USER_GUIDE.md)：登录、记录回忆、上传媒体、相册、回收站和备份导入导出。
- [管理员与部署维护手册](ADMIN_GUIDE.md)：本地运行、环境变量、Zeabur 部署、备份机制和故障排查。
- [OpenAI Sites 部署流程](SITES_DEPLOYMENT.md)：Sites 版 Worker 后端、D1/R2 绑定、构建打包、发布验证和常见问题。
...
- 云端导出优化：照片和视频按原始文件存入 ZIP，不重复压缩；Gunicorn 默认超时可通过 `GUNICORN_TIMEOUT` 调整，默认 600 秒。
- 导入范围：只接受本应用导出的 ZIP；导入时归属到当前登录账号，始终创建新的回忆和媒体记录，不覆盖现有内容，不恢复回收站状态。
- 大文件导入：导入时会从 ZIP 成员流式保存照片和视频，不再把整包或单个大视频一次性读入内存，也不再预扫描整个 ZIP，降低云端导入大备份时的超时和内存压力。
- Sites 大备份导入：浏览器只读取 ZIP 目录和所需成员，不会把整份 ZIP 作为一个请求上传；媒体逐个写入 R2，超过 24 MB 的单个媒体自动按 8 MB 分片上传，全部完成后再把回忆与媒体元数据发布到 D1。当前支持非 ZIP64、总大小低于 4 GB、最多 500 段回忆和 2000 个媒体文件的备份，可用 `npm run sites:import-smoke` 验证分片流程。
- Sites 托管构建：`npm run build` 会在干净环境中安装前端锁定依赖并生成 Worker/静态产物；本地已有依赖时可直接运行 `npm run sites:build`。
- 导入提示：导入成功后会清空旧错误提示，并阻止重复提交，避免页面同时显示失败和成功消息。
...
| `GUNICORN_TIMEOUT` | Gunicorn 请求超时时间，默认 600 秒 |
